...
def corporation_search(corporation,jurisdiction):
	...
	url = "https://api.opencorporates.com/v0.4/companies/search?q=%s" % urllib.parse.quote(corporation)
	url += "&jurisdiction_code=%s" % args['jurisdiction']
...
#
def corporate_filings(corporate_id,jurisdiction):

	api_url = "https://api.opencorporates.com/v0.4/companies/%s/%s/filings" % (jurisdiction,corporate_id)
...
def build_timeline(filings,corporate_name):
    ...
    with open(timeline_file,"w") as output:
